...
export default function NewEventForm({ setVisible }: { setVisible: Function }) {
  ...
  const fetchStores = async (longitude: number, latitude: number) => {
    let { data, error } = await supabaseClient.rpc("search_stores", {
      distance: 20,
  ...
  return (
    <>
      <form className="z-20 max-h-full w-full overflow-auto rounded bg-slate-600 p-10">
        ...
        <div className="-mx-3 flex flex-wrap">
          <button
            onClick={createGroup}
            className="mx-3 w-full items-center rounded-md bg-slate-800 py-2 px-5 text-center text-white shadow-md shadow-slate-700 hover:bg-slate-700"
          >
            Create Group
          </button>
        </div>
